...
def render_evaluation_tab(config_provider, provider_registry) -> None:
    ...
    avg_faithfulness = (
        sum(r.faithfulness_score for r in judged_records) / len(judged_records) if judged_records else 0.0
    )
    ...
    if judged_records:
        ...
        with col_j1:
            st.metric(
                "Avg Faithfulness (AI Judge)",
                f"{avg_faithfulness:.2f} / 5 ⭐",
                help="1 = unsupported, 5 = fully grounded in context",
            )
        with col_j2:
            st.metric(
                "Avg Relevance (AI Judge)",
                f"{avg_relevance:.2f} / 5 ⭐",
                help="1 = irrelevant, 5 = fully relevant to user query",
            )
    ...
    if not filtered_records:
        ...
        for i, r in enumerate(filtered_records):
            ...
            with st.expander(expander_title):
                ...
                st.markdown(
                    f"**Model:** `{r.model}` | "
                    f"**Retrieval Mode:** `{r.retrieval_mode}` | "
                    f"**Retrieved Chunks:** `{r.num_chunks}`"
                )
